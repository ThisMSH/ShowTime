...
<template>
    <!-- Delete modal toggle -->
    <button :id="`delete-btn-${trailer.id}`" type="button" class="flex items-center px-4 py-1 text-black bg-red-400 rounded-md gap-x-3 dark:bg-red-600 dark:text-white hover:bg-red-500 hover:dark:bg-red-700" :data-modal-target="`delete-trailer-${trailer.id}`" :data-modal-toggle="`delete-trailer-${trailer.id}`">
        <Icon class="text-lg" icon="material-symbols:delete-outline-rounded" />
        <span>Delete</span>
    </button>

    <!-- Main modal -->
    <div :id="`delete-trailer-${trailer.id}`" tabindex="-1"
        ...
        <div class="relative max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-slate-700">
                <button type="button"
                    ...
                    :data-modal-hide="`delete-trailer-${trailer.id}`">
                ...
                <div class="p-6 text-center">
                    ...
                    <h3 class="mb-5 text-lg font-normal text-slate-500 dark:text-slate-400">Are you sure you want to delete " <span class="font-semibold text-slate-800 dark:text-slate-200">{{ trailer.attributes.title }}</span> " from <span class="font-semibold text-slate-800 dark:text-slate-200">{{ trailer.relationships.show.title }}{{ trailer.relationships.show.season ? ' - ' + trailer.relationships.show.season : '' }}</span>?</h3>
                    <button @click="trailerStore.deleteTrailer(trailer.id)" type="button"
                        class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                        Yes, I'm sure
                    </button>
                    <button :data-modal-hide="`delete-trailer-${trailer.id}`" type="button"
