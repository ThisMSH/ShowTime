...
const addSub = async () => {
    await subtitleStore.addSubtitle(formData.value);
};
...
<template>
    <!-- Modal toggle -->
    <DefaultButton type="button" name="New Subtitle" icon-name="solar:subtitles-broken"
        data-modal-toggle="create-subtitle" />

    <!-- Main modal -->
    <div id="create-subtitle" tabindex="-1" aria-hidden="true"
                ...
                <form @submit.prevent="addSub">
                    <div class="grid grid-cols-1 gap-10 lg:grid-cols-2">
                        <TextInput v-model:input="formData.subName" label="Subtitle Name" inputType="text"
                            inputID="sub-name" :errors="subtitleStore.getErrors.subtitle_name" errorID="sub-name-error"
                            :isRequired="true" />
                        <ListEpisodesInput v-model:input="formData.epiID" label="Show" inputType="" inputID="show-id"
                            datalistID="shows-list" :showsList="subtitleStore.getAllSubtitles"
                            :errors="subtitleStore.getErrors.episode_id" errorID="show-error" :isRequired="true" />
                        <FileInput @file-content="getSubtitle" class="lg:col-span-2" label="Subtitle File" inputType="file"
                            inputID="sub-file" :errors="subtitleStore.getErrors.subtitle_file" errorID="sub-file-error"
                            :isRequired="true" />
