...
const updateRec = async () => {
    await recStore.updateRecommendation(props.rec.id, formData.value);
};
...
<template>
    ...
    <button type="button"
        ...
        :data-modal-toggle="`update-rec-${rec.id}`">
        <Icon class="text-lg" icon="material-symbols:edit-outline-rounded" />
        <span>Edit</span>
    </button>

    <!-- Main modal -->
    <div :id="`update-rec-${rec.id}`" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative w-full h-full max-w-2xl p-4 md:h-auto">
            ...
            <div class="relative p-4 bg-white rounded-lg shadow dark:bg-slate-800 sm:p-5">
                ...
                <div class="flex items-center justify-between pb-4 mb-4 border-b rounded-t sm:mb-5 dark:border-slate-600">
                    <h3 class="text-lg text-left whitespace-normal font-medium text-slate-900 dark:text-white">
                        Update a recommendation for " <span class="text-slate-800 dark:text-slate-300 font-semibold">{{ rec.relationships.show.title }}{{ rec.relationships.show.season ? ' - ' + rec.relationships.show.season : '' }}</span> ".
                    </h3>
                    <button type="button"
                ...
                <form @submit.prevent="updateRec">
                    <div class="grid grid-cols-1 gap-10 lg:grid-cols-2">
                        <ListInput v-model:input="formData.show_id" label="Show" inputType="" :inputID="`show-id-${rec.id}`"
                            :datalistID="`shows-list-${rec.id}`" :showsList="showStore.getAllShows?.shows" :errors="recStore.getErrors.show_id" :errorID="`show-error-${rec.id}`" />
                        <TextInput v-model:input="formData.color" label="Color" inputType="text" :inputID="`color-${rec.id}`" :errors="recStore.getErrors.color"
                        :errorID="`color-error-${rec.id}`" />
                        <FileInput @file-content="getLogo" class="lg:col-span-2" label="Logo" inputType="file" :inputID="`logo-${rec.id}`" :errors="recStore.getErrors.logo" :errorID="`logo-error-${rec.id}`" />
                        <FileInput @file-content="getCharacter" class="lg:col-span-2" label="Character" inputType="file" :inputID="`character-${rec.id}`" :errors="recStore.getErrors.character" :errorID="`character-error-${rec.id}`" />
