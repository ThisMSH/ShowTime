                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                        ...
                        <div>
                            <div class="relative z-0">
                                <label for="category_select" :class="[showStore.getErrors ? 'text-red-600' : 'text-slate-600', showStore.getErrors ? 'dark:text-red-500' : 'dark:text-slate-400']" class="absolute duration-300 transform -translate-y-6 scale-75 top-2 -z-10 origin-[0] peer-focus:left-0 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Category</label>
                                <select v-model="formData.category" id="category_select" :class="[showStore.getErrors ? 'border-red-600' : 'border-slate-600', showStore.getErrors ? 'dark:border-red-500' : 'dark:border-slate-400', showStore.getErrors ? 'focus:border-red-600' : 'focus:border-slate-600', showStore.getErrors ? 'dark:focus:border-red-500' : 'dark:focus:border-slate-400']" class="block py-2.5 px-2 w-full max-md:text-sm bg-transparent border-0 border-b-2 appearance-none child:bg-slate-100 child:dark:bg-slate-950 focus:outline-none focus:ring-0 peer">
