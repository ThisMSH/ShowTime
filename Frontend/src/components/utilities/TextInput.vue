...
<template>
    <div>
        <div class="relative z-0">
            <input v-model="inputComputed" :type="inputType" :id="inputID" :aria-describedby="errorID" :class="[errors ? 'border-red-600' : '', errors ? 'dark:border-red-500' : '', errors ? 'focus:border-red-600' : '', errors ? 'dark:focus:border-red-500' : '']" class="block py-2.5 px-0 w-full max-md:text-sm bg-transparent border-0 border-b-2 border-slate-600 appearance-none dark:border-slate-400 dark:focus:border-slate-400 focus:outline-none focus:ring-0 focus:border-slate-600 peer" placeholder=" " :readonly="isReadOnly" :autofocus="isFocused" :required="isRequired" />
