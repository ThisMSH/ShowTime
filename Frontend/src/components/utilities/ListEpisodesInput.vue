    ...
    <div>
        <div class="relative z-0">
            ...
            <datalist :id="datalistID">
                <template v-for="show in showsList" :key="show.id">
                    <option v-for="episode in show.relationships.episodes.list" :value="episode.id">{{ episode.attributes.number }} - {{ episode.attributes.title }} ({{ show.attributes.title }}{{ show.attributes.season ? ' - ' + show.attributes.season : '' }})</option>
                </template>
            </datalist>
        </div>
        <div v-if="errors">
