                    ...
                    <form @submit.prevent="authStore.handleLogin(formData)">
                        <div class="flex flex-col items-center justify-between gap-y-14">
                            <div class="w-64 flex flex-col gap-y-8">
                                <TextInput v-model:input="formData.email" label="Your E-mail" inputType="email" inputID="user_email" :errors="authStore.getErrors.email" errorID="user_email_error" :isFocused="true" />
                                <TextInput v-model:input="formData.password" label="Your password" inputType="password" inputID="user_password" :errors="authStore.getErrors.password" errorID="user_password_error" />
                            </div>
