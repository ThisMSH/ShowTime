...
<template>
    <TransitionGroup tag="section" name="home" appear>
        <!-- Top section -->
        <section key="s-1">
            <div class="relative h-home-tablet lg:h-home-desktop min-h-[600px]">
                ...
                <div
                    class="absolute top-0 left-0 w-full h-full opacity-90 bg-gradient-to-l from-black from-0% via-black via-100% md:via-60% to-transparent to-100%">
                </div>
                <div
                    class="p-10 absolute top-0 right-0 w-full md:w-3/5 h-full flex flex-col items-center justify-evenly text-3xl lg:text-4xl 2xl:text-5xl text-center text-slate-100 font-medium">
                    ...
                    <h1
                        class="bg-orange-500 bg-clip-text text-transparent text-shadow-slate-950-10 lg:text-shadow-slate-950-15 2xl:text-shadow-slate-950-20 text-stroke-orange-500-1 2xl:text-stroke-orange-500-2 font-logo text-6xl sm:text-7xl lg:text-8xl 2xl:text-9xl">
                        ShowTime</h1>
                    ...
                    <RouterLink v-if="!authStore.getUser" to="/register"
                        ...
                        <span
                            class="relative px-4 py-2 lg:px-5 lg:py-2.5 transition-all ease-in duration-75 bg-slate-950 rounded-l-full rounded-r-full group-hover:bg-opacity-0 text-xl">
        ...
        <section key="s-2" class="container mx-auto mt-40">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-10">
                <div class="relative">
                    <video class="absolute top-0.5 left-px w-[94%] h-[85%] md:h-[86%] lg:h-[84%] xl:h-[85%] object-cover"
                        autoplay loop muted poster="../../assets/images/other/covers.jpg">
                        <source src="../../assets/videos/index.mp4" type="video/mp4">
                    </video>
                ...
                <div class="relative">
                    <img class="absolute w-64 top-10 left-1/2 -translate-x-1/2" src="../../assets/images/other/epi_scene.png" alt="Sample from episodes">
                    <video class="absolute top-14 left-1/2 -translate-x-1/2 aspect-video w-64 object-cover" autoplay loop muted
                        poster="../../assets/images/other/covers.jpg">
                        <source src="../../assets/videos/index.mp4" type="video/mp4">
                    </video>
        ...
        <!-- Anime showcase -->
        <section key="s-3" class="container mx-auto mt-40">
            ...
            <div v-if="!recStore.recIsLoading" class="flex max-md:flex-col max-md:gap-y-32 items-center justify-around my-52">
                <ShowCard v-for="showCardContent in recStore.getAnime" :key="showCardContent.id"
                    :showCardContent="showCardContent" class="max-lg:scale-90" />
            </div>
        ...
        <section key="s-5">
            <div
                class="h-[600px] bg-[url('../../assets/images/other/Animes-low.png')] bg-no-repeat bg-cover bg-fixed bg-center">
            </div>
        </section>
        <!-- Live action showcase -->
        <section key="s-6" class="container mx-auto mt-40">
            <H2 title="Our Shows Recommendation" />
            <div v-if="!recStore.recIsLoading" class="flex max-md:flex-col max-md:gap-y-32 items-center justify-around my-52">
                <ShowCard v-for="showCardContent in recStore.getLiveAction" :key="showCardContent.id"
                    :showCardContent="showCardContent" class="max-lg:scale-90" />
            </div>
            <div v-else class="flex max-md:flex-col max-md:gap-y-32 items-center justify-around my-52">
                <ShowCardSkeleton v-for="i in 2" :key="i" />
        ...
        <section key="s-8">
            <div class="h-[600px] bg-[url('../../assets/images/other/Shows-low.png')] bg-no-repeat bg-cover bg-fixed bg-center">
            </div>
        </section>
        <!-- Q & A -->
        <section key="s-9" class="container mx-auto mt-40">
            <H2 title="Frequently Asked Questions" />
            <div
                class=" sm:w-3/4 md:w-2/3 lg:w-1/2 my-40 mx-3 sm:mx-auto rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700">
                <div id="accordion-color" data-accordion="collapse" data-inactive-classes="bg-slate-300 dark:bg-slate-800"
                    data-active-classes="bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-200">
                    <h4 class="text-xl" id="accordion-color-heading-1">
                        <button type="button"
                            class="flex items-center justify-between w-full p-5 font-medium text-left text-slate-500 border border-b-0 border-slate-200 dark:border-slate-700 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-600"
                    ...
                    <div id="accordion-color-body-1" class="hidden transition-all" aria-labelledby="accordion-color-heading-1">
                        <div
                            class="p-5 border border-b-0 border-slate-200 dark:border-slate-700 dark:bg-slate-900 transition-all">
                            <p class="text-slate-500 dark:text-slate-400">Lorem ipsum dolor sit amet consectetur adipisicing
                                elit. Ratione aliquam voluptates alias iure odio necessitatibus quis quisquam, consequuntur
                                enim. Culpa debitis necessitatibus suscipit voluptatem, ipsam eaque impedit totam deserunt
                                voluptate!</p>
                        </div>
                    </div>
                    ...
                    <div id="accordion-color-body-2" class="hidden transition-all" aria-labelledby="accordion-color-heading-2">
                        <div
                            class="p-5 border border-b-0 border-slate-200 dark:border-slate-700 dark:bg-slate-900 transition-all">
                            <p class="text-slate-500 dark:text-slate-400">Lorem ipsum, dolor sit amet consectetur adipisicing
                                elit. Quaerat corporis nam, quam dicta ipsa eos minus veritatis, quae tenetur laudantium
                                mollitia id distinctio sequi beatae ducimus tempora illum ipsam odio.</p>
                        </div>
                    </div>
                    <h4 class="text-xl" id="accordion-color-heading-3">
                        <button type="button"
                            class="flex items-center justify-between w-full p-5 font-medium text-left text-slate-500 border border-slate-200 dark:border-slate-700 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-600"
